...
- Browse albums in a responsive, keyboard-accessible grid ordered by album artist, trustworthy partial release date, then title. Deterministic offline placeholders reserve the cover layout without reading artwork yet; opening a card replaces the collection with a focused album detail and simple track list, while Back or Escape restores focus to that album without losing the current Library query or page. Searchable, paginated album-artist, genre, format, folder, and track views show useful context: artist/genre/format counts, folder album/track counts, or each track's album, number, format, duration, and file path. Album detail presents each track as a compact summary, then discloses readable technical facts and the full path before separately revealing raw normalized/native tags as Advanced metadata. Artist rows open exact filtered Albums; genre, format, and folder rows open exact filtered Tracks; track rows open the exact album and existing preview-only editor without proposing or applying a change. Search album, artist, genre, track, format, and path fields, or switch to a searchable scan-problem view.
...
- Use separate **Edit title** and **History & undo** album workspaces without losing the active draft or reviewed proposal. Album-title writes show per-file before/after values, move keyboard focus to confirmation and verification feedback, and keep API failures beside the recoverable workflow. History progressively discloses verified album, track, batch, and sequencing operations; every undo remains a new explicit preview and confirmation, reuses the same safe writer, and refuses conflicting external changes.
...
- Album cards intentionally use generated offline placeholders in this first Library hierarchy slice. Reading embedded or folder artwork through a narrow privileged boundary is deferred; the renderer receives no filesystem access and no network artwork is fetched.
