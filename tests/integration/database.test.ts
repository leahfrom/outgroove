...
describe("database migration and backup", () => {
  ...
  it("migrates an empty database and opens a verified backup", async () => {
    ...
    expect(source.connection.pragma("user_version", { simple: true })).toBe(8);
  ...
  it("upgrades the v7 track-edit schema without losing proposals or snapshots", async () => {
    const directory = await mkdtemp(join(tmpdir(), "outgroove-db-v7-"));
    temporary.push(directory);
    const path = join(directory, "catalog.sqlite3");
    const legacy = new Database(path);
