...
- Browse albums in a sandboxed React renderer using bounded SQLite pages and a rebuildable FTS-backed catalog search. Searchable, paginated album-artist, format, and track views show useful catalog context: artist and format counts, or each track's album, number, format, duration, and file path. Artist rows open an exact artist-filtered Albums view; format rows open an exact filtered Tracks view; track rows open the exact album and existing preview-only track editor without proposing or applying a change. Search album, artist, track, format, and path fields, or switch to a searchable scan-problem view.
