...
- See visible data-quality status and finding counts for every album on the current bounded Library page, then review deterministic local findings for missing or duplicate track numbers, internal sequence gaps, inconsistent album artists or partial release dates, missing dates, and exact scanner placeholders. Findings show affected files and only select the existing safe Workbench workflow; they never infer or apply a correction.
...
- Album diagnostics use normalized catalog tags and cannot determine the correct metadata, distinguish intentional numbering gaps from mistakes, or inspect unsupported/private frames. They are review prompts, not corrections. Library summary counts cover only the current bounded page; a whole-library data-quality filter is not implemented.
