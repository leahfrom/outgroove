...
- Store normalized and native tag views, technical properties, per-file failures, incremental scan signatures, restart-safe sync recovery journals, deduplicated artwork-undo assets, and rebuildable versioned provider responses in schema-v19 migrated SQLite.
...
- MusicBrainz candidate lookup is a separate, explicit network action and is not part of local scanning, browsing, or editing. It sends only the selected catalog album's title and album artist; paths, audio, artwork, native tags, fingerprints, track count, release date, and catalog numbers remain local. Successful raw responses are cached for 24 hours in schema-v19 with provider and response-schema metadata; a valid expired entry can be shown honestly as stale when the service is unavailable. Outgroove identifies itself, limits starts to one request per second, deduplicates identical in-flight requests, retries only bounded throttling responses, and validates every response before comparison. MusicBrainz search scores never become automatic Outgroove matches. Explicitly choosing a release prepares a local single-track or shared-field draft containing only effective Album artist, valid partial Release date, one unambiguous Catalog number, release ID, release-group ID, and one unambiguous release-artist ID. Ordered provider artist credits remain structured; title, track/disc totals, compilation, ambiguous multi-values, and track-specific IDs are not inferred. The draft still requires the normal preview, confirmation, snapshot, safe write, re-read, verification, and undo sequence. The current public endpoint integration is limited to this private non-commercial build; commercial distribution requires a MusicBrainz agreement or disabling/replacing the provider. See [ADR 0010](docs/decisions/0010-musicbrainz-album-candidates.md) and [ADR 0011](docs/decisions/0011-musicbrainz-release-tag-drafts.md).
