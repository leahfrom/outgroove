...
- Open **Radar** to search MusicBrainz for an artist by name, compare bounded ambiguous candidates by stable MusicBrainz identity and available type/country/area context, and explicitly save one as a favorite. Refresh one favorite explicitly to store a complete successful release snapshot, then review all current, certainly upcoming, certainly recent, or newly discovered release groups. Filter the local view by MusicBrainz primary type, including an honest unknown/unsupported bucket. Seen and dismissed state is durable, dismissed items can be restored, and a separate action opens the exact stored release-group identity on MusicBrainz. The local favorites and last successful Radar views remain available offline, survive catalog rebuilds, and are included in verified database backups. Removing a favorite requires a separate confirmation and also removes its local Radar history; it affects no audio, Library tags, provider cache, or DAP content.
