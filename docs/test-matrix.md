...
| MP3/FLAC safe replacement on native filesystem           | Passed on APFS fixture copies   | Automated NTFS + exclusive-lock CI test configured; manual packaged check pending | Unverified                           |
| Locked manifest-owned sync destination                   | Passed simulated failure test   | Real `FileShare.None` CI test configured; manual packaged check pending           | Unverified                           |
