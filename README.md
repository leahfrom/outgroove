...
- Store normalized and native tag views, technical properties, per-file failures, incremental scan signatures, restart-safe sync recovery journals, deduplicated artwork-undo assets, rebuildable versioned provider responses, durable favorite-artist identities, Radar snapshots, and opt-in Radar scheduling/notification preferences in schema-v23 migrated SQLite.
...
- Open **Identify recording with AcoustID** from one selected track to create a local Chromaprint preview. Outgroove shows the fingerprint algorithm, length, digest, and duration before a separate confirmation sends only the fingerprint and duration to AcoustID. Audio, paths, artwork, titles, artists, and existing tags are never uploaded. Candidates remain read-only and are never selected automatically; an explicit action may copy one returned MusicBrainz recording ID into the ordinary tag draft, which still requires the normal preview and confirmation before writing. The lookup needs a registered non-commercial application key at build time; see [AcoustID application setup](docs/acoustid.md).
