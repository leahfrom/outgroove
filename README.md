...
- Cover Art Archive preview is available only after MusicBrainz returns an exact release and the user explicitly loads its front cover. Main sends only that release UUID through a fixed HTTPS endpoint, follows redirects only to approved Cover Art Archive/Internet Archive hosts, validates bounded metadata and a maximum 2 MiB JPEG/PNG thumbnail, and gives the renderer only a locally encoded data URL. Raw provider metadata reuses the rebuildable schema-v19 provider cache; at most eight thumbnails are retained in memory for the current app session, and no remote image URL is exposed. Missing, invalid, throttled, cancelled, or unavailable responses fail within that candidate and never affect Library browsing. Outgroove does not fall back to another release or release group, persist the image, or offer it to the artwork writer. Cover Art Archive images may remain subject to third-party copyright; users must evaluate their right to use them. This integration is reviewed only for Outgroove's current private, non-commercial use. See [ADR 0013](docs/decisions/0013-read-only-cover-art-archive-preview.md).
