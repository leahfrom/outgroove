...
- Browse albums and tracks in a sandboxed React renderer using bounded SQLite pages and a rebuildable FTS-backed catalog search. Browse a searchable, paginated album-artist list with visible album/track counts and open an exact artist-filtered Albums view; search album, artist, track, format, and path fields, or switch to a searchable scan-problem view.
